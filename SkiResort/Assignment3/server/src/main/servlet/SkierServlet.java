  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (!validUrl(parts)) {
      ...
      this.skierDao.createLiftRide(Integer.valueOf(parts[8]), Integer.valueOf(parts[2]),
          Integer.valueOf(parts[4]), Integer.valueOf(parts[6]), time, liftID, verticalRise, this.cache);
      endTime = System.currentTimeMillis();
      latency = endTime - startTime;
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (!validUrl(parts)) {
      ...
      Integer totalVerticals = this.skierDao
          .getVerticalForSpecificDay(Integer.valueOf(parts[6]), Integer.valueOf(parts[8]), this.cache);
